...
func TestService_ValidateBlsToExecutionChange(t *testing.T) {
	...
	tests := []struct {
...

		{
			...
			clock: startup.NewClock(time.Now().Add(-time.Second*time.Duration(params.BeaconConfig().SlotTimeSchedule.SlotDuration(0)*10)), [32]byte{'A'}),
			setupSvc: func(s *Service, msg *ethpb.SignedBLSToExecutionChange, topic string) (*Service, string) {
				s.cfg.stateGen = stategen.New(beaconDB, doublylinkedtree.New())
				s.cfg.beaconDB = beaconDB
				s.cfg.clock = startup.NewClock(time.Now(), [32]byte{'A'})
		...
		},
		{
			...
			clock: startup.NewClock(time.Now().Add(-time.Second*time.Duration(params.BeaconConfig().SlotTimeSchedule.SlotDuration(0))*time.Duration(10)), [32]byte{'A'}),
			setupSvc: func(s *Service, msg *ethpb.SignedBLSToExecutionChange, topic string) (*Service, string) {
				s.cfg.stateGen = stategen.New(beaconDB, doublylinkedtree.New())
				s.cfg.beaconDB = beaconDB
				s.cfg.clock = startup.NewClock(time.Now(), [32]byte{'A'})
