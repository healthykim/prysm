...
func TestStore_NodeCount(t *testing.T) {
	f := setup(0, 0)
	state, blk, err := prepareForkchoiceState(t.Context(), f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, params.BeaconConfig().ZeroHash, 0, 0)
...
func TestStore_NodeByRoot(t *testing.T) {
	f := setup(0, 0)
	ctx := t.Context()
	state, blk, err := prepareForkchoiceState(t.Context(), f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, indexToHash(101), 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blk))
	state, blk, err = prepareForkchoiceState(t.Context(), f, 2, indexToHash(2), indexToHash(1), indexToHash(102), 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blk))
	emptyNode0 := f.store.emptyNodeByRoot[[32]byte{}]
	fullNode0 := f.store.treeRootNode
	emptyNode1 := fullNode0.children[0]
	fullNode1 := fullNode0.children[1]
	emptyNode2 := fullNode1.children[0]
	fullNode2 := fullNode1.children[1]

	expectedRoots := map[[32]byte]*Node{
		params.BeaconConfig().ZeroHash: emptyNode0,
		indexToHash(1):                 emptyNode1,
		indexToHash(2):                 emptyNode2,
	}
	...
	require.Equal(t, 3, len(f.store.fullNodeByPayload))
	expectedPayloads := map[[32]byte]*Node{
		[32]byte{}:       fullNode0,
		indexToHash(101): fullNode1,
		indexToHash(102): fullNode2,
	}

	for payloadHash, node := range f.store.fullNodeByPayload {
		v, ok := expectedPayloads[payloadHash]
...
func TestForkChoice_HasNode(t *testing.T) {
	f := setup(0, 0)
	ctx := t.Context()
	state, blkRoot, err := prepareForkchoiceState(t.Context(), f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
...
func TestStore_Head_BestDescendant(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(t.Context(), f, 2, indexToHash(2), indexToHash(1), params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(t.Context(), f, 3, indexToHash(3), indexToHash(1), params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(t.Context(), f, 4, indexToHash(4), indexToHash(2), params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	f.store.justifiedCheckpoint = &forkchoicetypes.Checkpoint{Epoch: 0, Root: indexToHash(1)}
	h, err := f.store.head(t.Context())
...
func TestStore_UpdateBestDescendant_ContextCancelled(t *testing.T) {
	...
	f := setup(0, 0)
	state, blkRoot, err := prepareForkchoiceState(ctx, f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	cancel()
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 2, indexToHash(2), indexToHash(1), params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
...
func TestStore_Insert(t *testing.T) {
	...
	treeRootNode := &Node{block: &BlockNode{slot: 0, root: indexToHash(0)}}
	nodeByRoot := map[[32]byte]*Node{indexToHash(0): treeRootNode}
	nodeByPayload := map[[32]byte]*Node{indexToHash(0): treeRootNode}
	jc := &forkchoicetypes.Checkpoint{Epoch: 0}
	fc := &forkchoicetypes.Checkpoint{Epoch: 0}
	fcs := setup(0, 0)
	s := &Store{emptyNodeByRoot: nodeByRoot, treeRootNode: treeRootNode, fullNodeByPayload: nodeByPayload, justifiedCheckpoint: jc, finalizedCheckpoint: fc}
	fcs.store = s
	payloadHash := [32]byte{'a'}
	ctx := t.Context()
	_, blk, err := prepareForkchoiceState(ctx, fcs, 100, indexToHash(100), indexToHash(0), payloadHash, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	assert.Equal(t, 2, len(s.emptyNodeByRoot), "Did not insert block")
	assert.Equal(t, (*Node)(nil), treeRootNode.block.parent, "Incorrect parent")
	assert.Equal(t, 1, len(treeRootNode.children), "Incorrect children number")
	assert.Equal(t, payloadHash, treeRootNode.children[0].block.payloadHash, "Incorrect payload hash")
	child := treeRootNode.children[0]
	assert.Equal(t, primitives.Epoch(1), child.block.justifiedEpoch, "Incorrect justification")
	assert.Equal(t, primitives.Epoch(1), child.block.finalizedEpoch, "Incorrect finalization")
	assert.Equal(t, indexToHash(100), child.block.root, "Incorrect root")
}
...
func TestStore_Prune_MoreThanThreshold(t *testing.T) {
	...
	f := setup(0, 0)
	ctx := t.Context()
	state, blkRoot, err := prepareForkchoiceState(ctx, f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, indexToHash(1001), 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	for i := uint64(2); i < numOfNodes; i++ {
		state, blkRoot, err = prepareForkchoiceState(ctx, f, primitives.Slot(i), indexToHash(i), indexToHash(i-1), indexToHash(1000+i), 0, 0)
		require.NoError(t, err)
		require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	}
...
func TestStore_Prune_MoreThanOnce(t *testing.T) {
	...
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	for i := uint64(2); i < numOfNodes; i++ {
		state, blkRoot, err = prepareForkchoiceState(ctx, f, primitives.Slot(i), indexToHash(i), indexToHash(i-1), params.BeaconConfig().ZeroHash, 0, 0)
		require.NoError(t, err)
		require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	}
...
func TestStore_Prune_NoDanglingBranch(t *testing.T) {
	f := setup(0, 0)
	ctx := t.Context()
	state, blkRoot, err := prepareForkchoiceState(ctx, f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, [32]byte{'1'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 2, indexToHash(2), params.BeaconConfig().ZeroHash, [32]byte{'2'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	...
	require.Equal(t, len(s.fullNodeByPayload), 1)
}
...
func TestStore_tips(t *testing.T) {
	...
	state, blkRoot, err := prepareForkchoiceState(ctx, f, 100, [32]byte{'a'}, params.BeaconConfig().ZeroHash, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 101, [32]byte{'b'}, [32]byte{'a'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 102, [32]byte{'c'}, [32]byte{'b'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 102, [32]byte{'j'}, [32]byte{'b'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 103, [32]byte{'d'}, [32]byte{'c'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 104, [32]byte{'e'}, [32]byte{'d'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 104, [32]byte{'g'}, [32]byte{'d'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 105, [32]byte{'f'}, [32]byte{'e'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 105, [32]byte{'h'}, [32]byte{'g'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 105, [32]byte{'k'}, [32]byte{'g'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 106, [32]byte{'i'}, [32]byte{'h'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(ctx, f, 106, [32]byte{'l'}, [32]byte{'k'}, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
...
func TestStore_PruneMapsNodes(t *testing.T) {
	...
	s := f.store
	s.finalizedCheckpoint.Root = indexToHash(1)
	require.Equal(t, 3, len(s.emptyNodeByRoot))
	require.Equal(t, 3, len(s.fullNodeByPayload))
	require.NoError(t, s.prune(t.Context()))
	require.Equal(t, len(s.emptyNodeByRoot), 1)
	require.Equal(t, len(s.fullNodeByPayload), 1)
...
func TestForkChoice_ReceivedBlocksLastEpoch(t *testing.T) {
	...
	ctx := t.Context()
	_, blk, err := prepareForkchoiceState(ctx, f, 1, [32]byte{'a'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 64, [32]byte{'A'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 65, [32]byte{'B'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 66, [32]byte{'C'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = uint64(time.Now().Add(time.Duration(-66*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second).Unix())
	...
	//       98
	// Received block last epoch is 1
	_, blk, err = prepareForkchoiceState(ctx, f, 98, [32]byte{'D'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	...
	//              132
	// Received block last epoch is 1
	_, blk, err = prepareForkchoiceState(ctx, f, 132, [32]byte{'E'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 99, [32]byte{'F'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = uint64(time.Now().Add(time.Duration(-132*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second).Unix())
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 100, [32]byte{'G'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = uint64(time.Now().Add(time.Duration(-132*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second).Unix())
	...
	_, blk, err = prepareForkchoiceState(ctx, f, 101, [32]byte{'H'}, b, b, 1, 1)
	require.NoError(t, err)
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = uint64(time.Now().Add(time.Duration(-132*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second).Unix())
...
func TestStore_TargetRootForEpoch(t *testing.T) {
	...
	state, blk, err := prepareForkchoiceState(ctx, f, params.BeaconConfig().SlotsPerEpoch, [32]byte{'a'}, params.BeaconConfig().ZeroHash, params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blk))
	...
	state, blk1, err := prepareForkchoiceState(ctx, f, params.BeaconConfig().SlotsPerEpoch+1, [32]byte{'b'}, blk.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	...
	state, blk2, err := prepareForkchoiceState(ctx, f, 2*params.BeaconConfig().SlotsPerEpoch+1, [32]byte{'c'}, blk1.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	...
	state, blk3, err := prepareForkchoiceState(ctx, f, 2*params.BeaconConfig().SlotsPerEpoch+2, [32]byte{'d'}, blk2.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	...
	state, blk4, err := prepareForkchoiceState(ctx, f, 3*params.BeaconConfig().SlotsPerEpoch, [32]byte{'e'}, blk1.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	...
	state, blk5, err := prepareForkchoiceState(ctx, f, 3*params.BeaconConfig().SlotsPerEpoch+1, [32]byte{'f'}, blk4.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	...
	state, blk6, err := prepareForkchoiceState(ctx, f, 4*params.BeaconConfig().SlotsPerEpoch+1, [32]byte{'g'}, blk5.Root(), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
...
func TestStore_CleanupInserting(t *testing.T) {
	f := setup(0, 0)
	ctx := t.Context()
	st, blk, err := prepareForkchoiceState(ctx, f, 1, indexToHash(1), indexToHash(2), params.BeaconConfig().ZeroHash, 0, 0)
	require.NoError(t, err)
