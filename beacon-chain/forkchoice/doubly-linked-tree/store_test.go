...
func TestForkChoice_ReceivedBlocksLastEpoch(t *testing.T) {
	...
	s.genesisTime = time.Now().Add(time.Duration(-1*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	ctx := t.Context()
	...
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = time.Now().Add(time.Duration((-64*int64(params.BeaconConfig().SecondsPerSlot))-1) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
	require.Equal(t, uint64(1), count)
	...
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = time.Now().Add(time.Duration(-66*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
	...
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = time.Now().Add(time.Duration(-98*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
	require.Equal(t, uint64(1), count)
	...
	_, err = s.insert(ctx, blk, 1, 1)
	require.NoError(t, err)
	s.genesisTime = time.Now().Add(time.Duration(-132*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
	require.Equal(t, uint64(1), count)
	...
	s.genesisTime = time.Now().Add(time.Duration(-134*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
	require.Equal(t, uint64(1), count)
	s.genesisTime = time.Now().Add(time.Duration(-165*int64(params.BeaconConfig().SecondsPerSlot)) * time.Second)
	count, err = f.ReceivedBlocksLastEpoch()
	require.NoError(t, err)
