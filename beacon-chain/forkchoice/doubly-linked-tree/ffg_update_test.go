...
func TestFFGUpdates_OneBranch(t *testing.T) {
	...
	state, blkRoot, err := prepareForkchoiceState(t.Context(), f, 1, indexToHash(1), params.BeaconConfig().ZeroHash, indexToHash(101), 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(t.Context(), f, 2, indexToHash(2), indexToHash(1), indexToHash(102), 1, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(t.Context(), f, 3, indexToHash(3), indexToHash(2), indexToHash(103), 2, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
