...
func TestServer_ListValidators_reqStateIsNil(t *testing.T) {
	beaconDB := dbTest.SetupDB(t)
	sg, err := params.BeaconConfig().SlotTimeSchedule.SinceGenesis(params.BeaconConfig().SlotsPerEpoch)
	require.NoError(t, err)
...
func TestServer_ListValidators_FromOldEpoch(t *testing.T) {
	...
	sg, err := params.BeaconConfig().SlotTimeSchedule.SinceGenesis(slots.UnsafeEpochStart(epochs))
	require.NoError(t, err)
	gt := time.Now().Add(-1 * sg)
	bs := &Server{
		HeadFetcher: &mock.ChainService{
