...
func TestStore_OnAttestation_Ok_DoublyLinkedTree(t *testing.T) {
	eval := func(ctx context.Context, service *Service, genesisState state.BeaconState, pks []bls.SecretKey) {
		service.SetGenesisTime(time.Unix(time.Now().Unix()-int64(params.BeaconConfig().SlotTimeSchedule.SlotDuration(0)), 0))
...
func TestAttEpoch_MatchPrevEpoch(t *testing.T) {
	...
	nowTime := time.Unix(int64(params.BeaconConfig().SlotsPerEpoch)*int64(params.BeaconConfig().SlotTimeSchedule.SlotDuration(0)), 0)
...
func TestAttEpoch_NotMatch(t *testing.T) {
	...
	nowTime := time.Unix(2*int64(params.BeaconConfig().SlotsPerEpoch)*int64(params.BeaconConfig().SlotTimeSchedule.SlotDuration(0)), 0)
