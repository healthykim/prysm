...
func (s *Service) CanSubscribe(topic string) bool {
	...
	epbsForkDigest, err := forks.ForkDigestFromEpoch(params.BeaconConfig().EPBSForkEpoch, s.genesisValidatorsRoot)
	if err != nil {
		log.WithError(err).Error("Could not determine EPBS fork digest")
		return false
	}
	switch parts[2] {
	...
	case fmt.Sprintf("%x", epbsForkDigest):
	default:
		return false
	}
